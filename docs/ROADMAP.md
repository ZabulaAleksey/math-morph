...
**Текущий статус:** этапы 001–091 реализованы и проверены на ветке `feature/stage-091`; работы нет. Следующий ещё не начатый этап — 092 (`experimental MathType adapter`). Подробные доказательства и статус `verified` находятся только в `docs/TRACEABILITY.md`.
