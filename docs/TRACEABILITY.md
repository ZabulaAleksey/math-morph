...
| FR/NFR/SEC-SEMANTIC-105 | 105 | iterative redacted SCC diagnostics in `semantic_diagnostics.rs`; caller-owned node/edge work limits checked before SCC allocation, public callable self-cycle, scalar/forward undefined regressions and private synthetic multi-node SCC proof (unreachable through current strict visible-before graph builder) | verified with evidence boundary |
| FR/NFR/SEC-SUBSTITUTE-106..111 | 106–111 | `specs/features/substitution-and-evaluation-display.spec.md`; full lexical AST traversal, visible-before once/recursive substitution, cumulative input/output/expansion/trace limits, redacted success/failure trace, display modes, precision policy, 12 targeted regressions и clean independent/security review | verified |
...
| SPEC-07 комплексные числа | 112–122 | `specs/features/complex-numbers.spec.md`; finite algebraic/polar model, tolerance round-trip, Cartesian operations, scaled extreme division, bounded typed polar/division trace, precision-aware signed-zero-normalized output, 10 targeted regressions и clean independent/security review | verified |
| FR-PLOT-123/124, FR-PREVIEW-126, FR-FALLBACK-127 | 123–127 | `specs/features/plot-diagram-evidence-gates.spec.md`; подтверждённый PlotRegion → strict Document IR V3 metadata, V1 compatibility, safe partial projection и explicit PNG/JPEG DOCX preview component; stage 125 live extraction blocked by fixtures | verified с evidence boundary |
| SPEC-08 ChartIR/Excel | 128–132 | Требуются legal versioned plot fixtures, series/axes mapping и Excel exporter security contract | blocked |
| FR-PREVIEW-134 / SPEC-09 | 133–142 | Explicit DiagramIr PNG/JPEG preview component проверен; detection/forensics и expanded DiagramIR/VSDX требуют format/live evidence | blocked, component 134 verified |
