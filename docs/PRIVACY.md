...
## Fallback между режимами доверия

Переход с local/WASM processing на server-side processing
изменяет privacy boundary и не является прозрачным техническим fallback.

Такой переход допускается только если он явно разрешён product contract
и понятен пользователю.

Ошибка локальной обработки не разрешает автоматически загружать документ на сервер.