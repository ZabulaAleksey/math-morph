...
<!-- MATHMORPH-E2E-READINESS -->
## Готовность E2E

E2E-уровень тестирования не считается выполненным до появления реального production-пути:
`CLI/API → backend converter → production worksheet→Document-IR`.

P0:
- Валидная загрузка `.xmcd`/`.mcdx` и получение редактируемого DOCX.
- Безопасная обработка повреждённого или небезопасного ввода с локализованной ошибкой без создания артефакта и утечки данных.

Ожидается в дальнейшем:
- Прогресс асинхронной конвертации, восстановление после обрыва сети и возобновление загрузки.
- Аутентификация, 2FA, восстановление доступа и корректная обработка API-областей.
- Истечение signed URL, смена сети и возобновление скачивания.

Если backend/API/CLI путь для какого-либо сценария отсутствует, сценарий оставляй с пометкой
`BLOCKED_BY_BACKEND_<имя-сценария>` и не считай его закрытым до появления end-to-end цепочки.
