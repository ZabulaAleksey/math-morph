...
MathMorph использует Calm Blue UI как утверждённый канонический visual baseline будущего web-интерфейса. Первый продуктовый путь — безопасная загрузка `.xmcd`/`.mcdx`, настройка конвертации и получение редактируемого DOCX/OMML. На момент утверждения UI flow ещё не реализован: документ задаёт требования для последующей реализации, а не описывает готовые экраны. Система основана на desktop-референсе светлого интерфейса с боковой навигацией и карточками объектов размером 1800 × 822 px, но не наследует его бренд, предметную область или локализацию.
